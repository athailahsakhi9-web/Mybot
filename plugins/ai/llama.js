...
const pluginConfig = {
    name: 'llama',
    alias: ['llamaai', 'llama3', 'metaai'],
    category: 'ai',
    description: 'Tanya jawab dengan Llama AI',
    usage: '.llama <pertanyaan/teks>',
    example: '.llama apa itu php',
    cooldown: 5,
    energi: 1,
    isEnabled: true
}
...
async function handler(m, { sock, args, config }) {
    ...
    try {
        // Panggil API Llama AI NexaDev (Timeout 2 Menit = 120.000 ms)
        const apiUrl = `${baseUrl}/ai/llama?q=${encodeURIComponent(query.trim())}`
        ...
        if (!json || json.status === false) {
            throw new Error(json?.message || json?.error || 'Gagal mengambil respon dari Llama AI')
        }
