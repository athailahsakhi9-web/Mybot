...
const pluginConfig = {
    name: 'deepsek',
    alias: ['deepseek', 'deepseekai', 'dpsek'],
    category: 'ai',
    description: 'Tanya jawab dengan DeepSeek AI',
    usage: '.deepsek <pertanyaan/teks>',
    example: '.deepsek apa itu nodejs',
    cooldown: 5,
    energi: 1,
    isEnabled: true
}
...
async function handler(m, { sock, args, config }) {
    ...
    try {
        // Panggil API DeepSeek AI NexaDev (Timeout 2 Menit = 120.000 ms)
        const apiUrl = `${baseUrl}/ai/deepsek?text=${encodeURIComponent(text.trim())}`
        ...
        if (!json || json.status === false) {
            throw new Error(json?.message || json?.error || 'Gagal mengambil respon dari DeepSeek AI')
        }
