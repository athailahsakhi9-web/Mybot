...
// ─── Sticker EXIF helpers (diambil dari smeta) ─────────────────────────────────
...
function setWebpExif(webpBuffer, metadata, fallbackWidth = 512, fallbackHeight = 512) {
    ...
    // Ini akar masalah "tidak bisa melihat info stiker": tanpa bit ini, WA anggap file korup.
...
const pluginConfig = {
    ...
    alias:       ['brattext'],
    category:    'sticker',
    description: 'Membuat sticker brat (dengan metadata EXIF custom)',
    usage:       '.brat <text> | <packname> | <author>',
    example:     '.brat Hai semua | Nexa Bot | nexadev',
    isOwner:     false,
    isPremium:   false,
    isGroup:     false,
    isPrivate:   false,
...
async function handler(m, { sock }) {
    ...
    if (!raw) {
        return m.reply(
            ...
            `\`${m.prefix}brat Hai semua | Nexa Bot | nexadev\``
        )
    }
    ...
    try {
        ...
        // ─── Tambahkan metadata EXIF ala smeta ─────────────────────────────
        const metadata = {
